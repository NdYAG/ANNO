...
angular.module("ANNO.directives", []).directive("inputEnter", function() {
    var ENTER_KEY = 13;
    return function(scope, elem, attrs) {
        elem.bind("keydown", function(event) {
            event.keyCode === ENTER_KEY && scope.$apply(attrs.inputEnter);
        });
    };
        ...
        controller: function($scope) {
            var utils = {
                bold: function(s, editor) {
                    editor.replaceSelection("*" === s.charAt(0) && s.charAt(s.length - 1 === "*") ? s.replace(/\*/g, "") : "**" + s.replace(/\*/g, "") + "**");
                },
                italic: function(s, editor) {
                    editor.replaceSelection("_" === s.charAt(0) && s.charAt(s.length - 1 === "_") ? s.replace(/_/g, "") : "_" + s.replace(/_/g, "") + "_");
                },
                quote: function(s, editor) {
                    editor.replaceSelection(">" === s.charAt(0) ? util.lTrim(s.replace(/\>/g, "")) : "> " + s.replace(/\>/g, ""));
                }
            }, self = this;
            this.codeMirrorLoad = function(editor) {
                $scope.bold = function() {
                    "" !== editor.getSelection() && utils.bold(editor.getSelection(), editor);
                }, $scope.italic = function() {
                    "" !== editor.getSelection() && utils.italic(editor.getSelection(), editor);
                }, $scope.quote = function() {
                    "" !== editor.getSelection() && utils.quote(editor.getSelection(), editor);
                }, self.uploadPhoto = function(dataURL, file) {
                    $scope.article.last_photo = $scope.article.last_photo || 0, ++$scope.article.last_photo, 
                    $scope.images.push({
    ...
    return {
        ...
        link: function(scope, elem, attrs, ctrls) {
            var editorCtrl = ctrls[0], ngModel = ctrls[1], editor = new CodeMirror(elem[0], {
                mode: "gfm",
                lineWrapping: !0,
                lineNumbers: !1,
                matchBrackets: !0,
            ...
            editor.refresh(), editor.on("change", function(cm) {
                var phase = scope.$root.$$phase;
                "$apply" == phase || "$digest" == phase ? ngModel.$setViewValue(cm.getValue()) : scope.$apply(function() {
                    ngModel.$setViewValue(cm.getValue());
                });
            }), ngModel.$render = function() {
                ngModel.$viewValue && editor.setValue(ngModel.$viewValue);
        ...
        link: function(scope, elem) {
            ...
            renderer.image = function(href, title, text) {
                ...
                if (this.options.sanitize) {
                    ...
                    if (0 === prot.indexOf("javascript:")) return "";
                }
                var out = '<a href="' + href + '"';
                return title && (out += ' title="' + title + '"'), out += ' target="_blank" >' + text + "</a>";
            }, marked.setOptions({
                renderer: renderer,
                gfm: !0,
                tables: !0,
                ...
                if (!_.isUndefined(text)) {
                    ...
                    Array.prototype.slice.call(elem.find("pre"), 0).forEach(function(block) {
                        var $block = $(block), $code = $block.find("code");
                        if ($code.attr("class").length) {
                            var mode = $code.attr("class").slice(5);
                            mode = MIME[mode] || mode, CodeMirror.runMode($block.text(), mode, block), $block.addClass("cm-s-prose-bright");
                        }
                    }), Array.prototype.slice.call(elem.find("img"), 0).forEach(function(img) {
                        var $img = $(img), source = $img.attr("data-src");
                        imageCache[source] ? $img.attr("src", imageCache[source]) : $http.get($img.attr("data-src"), {
                            responseType: "blob",
                            cache: !0
                        }).success(function(blob) {
                            imageCache[source] = window.URL.createObjectURL(blob), Array.prototype.slice.call(elem.find("img"), 0).forEach(function(new_img) {
    ...
    return function(scope, elem) {
        scope.$on("loading:show", function() {
            ...
            elem.addClass("fadeOut"), $timeout(function() {
    ...
    return function(scope, elem) {
        ...
        scope.$on("alert:error", function(e, data) {
            ...
            msg || (msg = data.code ? "你遇到了编号为" + data.code + '的错误"' + data.msg : '发生了奇怪的错误，可能是服务器不稳定，稍后再试试吧。<a href="http://www.douban.com/doumail/write?to=1662222" target="_blank">告诉管理员让他修复错误吧。</a>'), 
            elem.addClass("error").addClass("show").html(msg);
        }), scope.$on("alert:success", function(e, msg) {
            elem.addClass("success").addClass("show").html(msg), $timeout(function() {
            ...
            elem.removeClass("show");
        }), scope.$on("alert:error:hide", elem.removeClass.bind(elem, "show")), $rootScope.$on("$routeChangeStart", scope.$emit.bind(scope, "alert:error:hide"));
    };
    ...
    return {
        ...
        link: function(scope, elem, attrs, ngModel) {
            var colorThief = new ColorThief(), sourceImage = new Image();
            scope.$watch(attrs.image, function(url) {
                url && ($http.get(url, {
                    ...
                    var rgb = colorThief.getColor(sourceImage), luma = .2126 * rgb[0] + .7152 * rgb[1] + .0722 * rgb[2];
...
}).directive("remoteImage", [ "$http", "$compile", function($http) {
    function fetchImage(scope, elem, url) {
        $http.get(url, {
            responseType: "blob",
            cache: !0
        }).success(function(blob) {
            "IMG" === elem[0].nodeName ? elem.attr("src", window.URL.createObjectURL(blob)) : elem.css({
